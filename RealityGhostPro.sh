...
async function X(){await F();u();var s=document.getElementById('rw1'),d=S.disk||{},r=S.ram||{},c=S.cpu||{},l=S.load||{};var cpuUsage=typeof c==='string'?parseFloat(c):(c.usage||0),cpuCores=typeof c==='string'?'—':(c.cores||'—');s.innerHTML='';var a=[{l:S.online?'✅':'❌',n:'Status',v:S.online?'Online':'Offline',s:''},{l:'💾',n:'Disk',v:(typeof d.used==='string'?parseFloat(d.used):((d.used||0)/1024)).toFixed(1)+'/'+(typeof d.total==='string'?parseFloat(d.total):((d.total||0)/1024)).toFixed(1)+' GB',s:''},{l:'🧠',n:'RAM',v:r.total+' MB',s:r.usage?r.usage.toFixed(1)+'%':''},{l:'⚙️',n:'CPU',v:cpuUsage?cpuUsage.toFixed(1)+'%':'—',s:''},{l:'📊',n:'Cores',v:cpuCores,s:''},{l:'🔗',n:'Conn',v:S.connections||0,s:''},{l:'📌',n:'Xray',v:S.xray_version||'—',s:''}];a.forEach(function(t){s.innerHTML+='<div class=cd><div class=cd-l>'+t.l+' '+t.n+'</div><div class="cd-v '+(t.v&&t.v.length>8?'xs':'')+'">'+t.v+(t.s?'<small>'+t.s+'</small>':'')+'</div></div>'})}
async function Y(){await F();var s=document.getElementById('sys_rs'),d=S.disk||{},r=S.ram||{},c=S.cpu||{};var cpuUsage=typeof c==='string'?parseFloat(c):(c.usage||0);var cpuCores=typeof c==='string'?'—':(c.cores||'—');var diskUsed=typeof d.used==='string'?parseFloat(d.used):((d.used||0)/1024);var diskTotal=typeof d.total==='string'?parseFloat(d.total):((d.total||0)/1024);var diskPct=d.usage||(diskUsed&&diskTotal?(diskUsed/diskTotal*100).toFixed(1):0);var h='<div class=rl><span>💾 Disk</span><b>'+diskUsed.toFixed(1)+'/'+diskTotal.toFixed(1)+' GB</b></div><div class=bc><div class="bf p" style=width:'+diskPct+'%></div></div>';h+='<div class=rl><span>🧠 RAM ('+r.total+' MB)</span><b>'+((r.used||0)).toFixed(0)+' MB / '+(r.usage||0).toFixed(1)+'%</b></div><div class=bc><div class="bf gr" style=width:'+(r.usage||0)+'%></div></div>';h+='<div class=rl><span>⚙️ CPU ('+(cpuCores)+' cores)</span><b>'+cpuUsage.toFixed(1)+'%</b></div><div class=bc><div class="bf yl" style=width:'+cpuUsage+'%></div></div>';s.innerHTML=h;document.getElementById('upt').textContent=S.uptime?'Up '+S.uptime:'—'}
