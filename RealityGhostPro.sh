...
function si(){var s=document.getElementById('srv_ir'),i=S.server||{};if(!i||Object.keys(i).length===0){s.innerHTML='<div style=color:var(--t2);font-size:11px;text-align:center;padding:10px>—</div>';return}s.innerHTML='';Object.keys(i).forEach(function(k){s.innerHTML+='<div class=irow><span class=irow-l>'+k+'</span><span class=irow-v>'+(i[k]||'—')+'</span></div>'})}
function ql(){var s=document.getElementById('qa'),l=S.links||[];if(!l||l.length===0){s.innerHTML='<span style=color:var(--t2);font-size:10px>—</span>';return}l.forEach(function(t){s.innerHTML+='<span class=chp onclick=window.open("'+t.url+'")>'+t.label+'</span>'})}
